...
ShowKnowledge::ShowKnowledge(QWidget *parent)
    : QMainWindow(parent)
    , ui(new Ui::ShowKnowledge)
{
    ui->setupUi(this);
    ...
    connect(timer,&QTimer::timeout,this,&ShowKnowledge::updateProgress);
    ...
    //布局调整
    QVBoxLayout *mainLayout = qobject_cast<QVBoxLayout*>(ui->centralWidget->layout()); //两种界面
    ...
    //信号连接
    //作业数量变化时更新表格
    connect(ui->spinBox,QOverload<int>::of(&QSpinBox::valueChanged),this,&ShowKnowledge::updateJobTable);
    //模式切换事件
    connect(modeGroup,QOverload<QAbstractButton*>::of(&QButtonGroup::buttonClicked),this,&ShowKnowledge::on_modeChanged);
}
...
void ShowKnowledge::on_startButton_clicked()
{
...
void ShowKnowledge::on_pauseButton_clicked()
{
    timer->stop();
}

void ShowKnowledge::on_continueButton_clicked()
{
...
//更新模式
void ShowKnowledge::on_modeChanged()
{
...
void ShowKnowledge::updateJobTable(int count)
{
    //仅仅在手动模式下才能运行
...
void ShowKnowledge::createJobItems()
{
    //清空所有图形项
    scene->clear();
    // scene = new QGraphicsScene(this);
...
void ShowKnowledge::generateJobs(int n)
{
    //清空所有作业列表
    jobs.clear();
    ...
    if(manualMode->isChecked()) {
        ...
        // 随机模式保持原有逻辑
        QList<QPair<int, int>> tempJobs;
        for(int i = 0; i < n; ++i) {
            int arrival = QRandomGenerator::global()->bounded(0, n);
            int burst = QRandomGenerator::global()->bounded(1, 10);
            tempJobs.append(qMakePair(arrival, burst));
        }
        std::sort(tempJobs.begin(), tempJobs.end(),
                  [](const QPair<int, int>& a, const QPair<int, int>& b) {
                      return a.first < b.first;
                  });
        for(int i = 0; i < tempJobs.size(); ++i) {
            jobs.append(Job(i, tempJobs[i].first, tempJobs[i].second));
        }
        ...
        qreal prevEnd = 0;
        for(Job& job : jobs) {
            job.startTime = qMax(prevEnd, (qreal)job.arrivalTime);
            job.endTime = job.startTime + job.burstTime;
            prevEnd = job.endTime;
        }
        maxEndTime = prevEnd;
...
void ShowKnowledge::createArrow(qreal x, qreal y, Qt::GlobalColor color, Job& job, bool isArrival)
{
...
void ShowKnowledge::updateProgress()
{
...
void ShowKnowledge::closeEvent(QCloseEvent *event) {
    emit(close());
    event->accept();
}
